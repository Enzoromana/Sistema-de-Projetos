        ...
        <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md transition-opacity duration-300 ${closing ? 'opacity-0' : 'animate-in fade-in duration-500'}`}>
            <div className={`bg-white rounded-[1.5rem] md:rounded-[2.5rem] w-full max-w-lg shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] border border-slate-100 overflow-hidden transition-all duration-300 ${closing ? 'scale-95 opacity-0' : 'animate-in zoom-in duration-500'}`}>
                ...
                <div className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-800 p-8 md:p-12 text-white overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 w-48 h-48 bg-purple-400/10 rounded-full -ml-12 -mb-12 blur-2xl"></div>
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-indigo-100 text-sm md:text-base font-medium leading-relaxed">
                            O Hub Manager agora funciona perfeitamente no seu celular. Acesse todos os módulos de qualquer lugar.
                        </p>
                    </div>
                </div>
                ...
                <div className="p-6 md:p-10 space-y-5">
                    <div className="space-y-3">
                        {[
                            { icon: <Smartphone size={18} />, text: 'Interface 100% adaptada para mobile', color: 'bg-purple-50 text-purple-600' },
                            { icon: <Menu size={18} />, text: 'Menu hamburger com acesso rápido', color: 'bg-indigo-50 text-indigo-600' },
                            { icon: <LayoutGrid size={18} />, text: 'Todos os módulos acessíveis no celular', color: 'bg-teal-50 text-teal-600' },
                            { icon: <CheckCircle2 size={18} />, text: 'Login, Projetos, Salas, Auditoria otimizados', color: 'bg-emerald-50 text-emerald-600' },
                        ].map((item, i) => (
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-2">
                        <button
                            onClick={viewAll}
                            className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-xl shadow-indigo-200 text-sm"
                        >
